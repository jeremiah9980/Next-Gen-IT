...
def _normalize_domain(raw: Any) -> str | None:
    """Best-effort domain extraction that is safe against bad input.

    - Strips ``http(s)://`` and ``www.`` as *prefixes* (not character sets).
    - If given a full URL, uses ``urlparse`` to pull the hostname.
    - Drops path/query/port fragments so ``example.com/foo`` → ``example.com``.
